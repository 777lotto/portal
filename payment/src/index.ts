...
async function sendPaymentReminder(
  ...
  try {
    ...
    const amountFormatted = (service.price_cents / 100).toFixed(2);
    ...
    if (reminder.reminder_count === 0) {
      // First reminder
      message = `Hi ${service.name}, this is a friendly reminder that your invoice for $${amountFormatted} is due. You can pay online here: ${paymentLink} or reply PAY to get payment assistance.`;
    }
    else if (reminder.reminder_count === 1) {
      // Second reminder
      message = `Hi ${service.name}, your invoice for $${amountFormatted} is now past due. Please pay online at ${paymentLink} or reply PAY for payment options.`;
    }
    else {
      // Final reminder
    ...
    // Send the SMS via the notification worker
...
async function runPaymentReminders(env: Env): Promise<{ success: boolean; count: number }> {
  try {
    ...
    for (const service of services) {
      const result = await sendPaymentReminder(env, service.id);
      if (result.success) {
        successCount++;
      }
    }
...
export async function markPaymentReminderPaid(env: Env, serviceId: number): Promise<void> {
  try {
    await env.DB.prepare(
      `UPDATE payment_reminders
       SET status = 'paid'
