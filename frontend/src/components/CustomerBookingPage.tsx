...
function CustomerBookingPage() {
  ...
  return (
    <div className="container mt-4">
      <h1 className="text-3xl font-bold mb-4">Schedule Your Next Service</h1>
      <CalendarLegend />
      ...
      <BookingCalendar onSelectSlot={handleSelectSlot} isCustomer={true} />
      {selectedDate && user && (
        <BookingModal
          isOpen={modalOpen}
          onClose={() => setModalOpen(false)}
          selectedDate={selectedDate}
          user={user} // Pass user data to pre-populate the form
        />
      )}
    </div>
  );
